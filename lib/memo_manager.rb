...
class MemoManager < StorageManager
  def self.read(user_id)
    res = @@conn.exec('SELECT memos.id, memos.title, memos.body FROM memos WHERE user_id = $1', [user_id])
    user_memos = {}
    res.each do |row|
  ...
  def self.save(user_id:, title:, body:)
    res = @@conn.exec(
  ...
  def self.find_by(user_id, target_id)
    res = @@conn.exec(
